...
function s.filter2(c,e,tp,mc,rk)
	...
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk) and c:IsSetCard(0x7cc) and mc:IsCanBeXyzMaterial(c,tp)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	--If you LP is 2000 higher than yours opponent
function s.tdcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetLP(tp)<=Duel.GetLP(1-tp)-2000
end
function s.tdfilter(c)
	return c:IsSetCard(0x7cc) and c:IsType(TYPE_MONSTER)
end
...
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.tdfilter,tp,LOCATION_DECK,0,1,1,nil):GetFirst()
	if tc then
		Duel.ShuffleDeck(tp)
