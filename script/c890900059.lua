...
function s.initial_effect(c)
	--synchro summon
	Synchro.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x3dd),1,1,s.sfilter,1,99)
	c:EnableReviveLimit()
	--spsummon
...
function s.sfilter(c,scard,sumtype,tp)
	return c:IsSummonCode(scard,sumtype,tp,890900014) or (c:IsHasEffect(890900042) and not c:IsType(TYPE_TUNER)) or (c:IsHasEffect(890900032) and not c:IsType(TYPE_TUNER))
end
...
function s.spfilter2(c,e,tp)
	return c:IsSetCard(0x3de) and c:IsLevel(7) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
