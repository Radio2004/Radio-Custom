...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(eg) do
		if tc:IsSetCard(0x7cc) and tc:IsType(TYPE_XYZ) and tc:IsReason(REASON_DESTROY) and tc:IsReason(REASON_BATTLE) then
...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsSetCard(0x7cc) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:GetFlagEffect(id)~=0
