--Melirria, Episode of Fire and Ice - The Great Witch
	...
	local mi,ma=c:GetTributeRequirement(c)
	if mi<minc then mi=minc end
	if ma<mi then return false end
	return ma>0 and Duel.IsExistingMatchingCard(s.castlefilter,c:GetControler(),LOCATION_MZONE,0,3,nil,c:GetControler(),mi,ma)
end
function s.sumtg(e,tp,eg,ep,ev,re,r,rp,chk,c)
	tp=c:GetControler()
	local mi,ma=c:GetTributeRequirement(c)
	local sg=Duel.IsExistingMatchingCard(tp,s.castlefilter,tp,LOCATION_MZONE,0,3,3,true,nil,tp,mi,ma)
	if sg then
		sg:KeepAlive()
		e:SetLabelObject(sg)
		return true
